...
class LabelSpec:
    ...
    def __post_init__(self) -> None:
        if self.num_classes <= 0:
            raise ValueError("labels.num_classes must be > 0")
        if self.names is not None and len(self.names) != self.num_classes:
            raise ValueError("labels.names must match num_classes")
...
def make_multi_group_factory(registry: ModelGroupRegistry, group_names: Iterable[str]) -> BatchFactory:
    ...
    def factory(batch_size: int, device: torch.device, seed: int) -> HtSBatch:
        ...
        idx = int(torch.randint(0, len(names), (1,), generator=gen).item())
        name = names[idx]
