...
def train_classifier(
    ...
    eval_batch_fn: Optional[Callable[[int, torch.device, int], object]] = None,
) -> TrainLog:
    ...
    eval_batch_fn = eval_batch_fn or batch_fn
    ...
    for step in range(1, config.steps + 1):
        ...
        if step == 1 or step % config.eval_every == 0 or step == config.steps:
            with torch.no_grad():
                train_acc = accuracy(logits, batch.labels)
                model.eval()
                eval_batch = eval_batch_fn(config.batch_size, device, config.seed * 2_000_000 + step)
                eval_logits = model(
                    eval_batch.input_ids,
                    eval_batch.task_ids,
                    getattr(eval_batch, "attention_mask", None),
                )
                eval_loss_bd = objective(model, eval_logits, eval_batch.labels, step=step)
                eval_acc = accuracy(eval_logits, eval_batch.labels)
                model.train()
            row: Dict[str, float | int | str] = {
                ...
                "eval_loss": float(eval_loss_bd.loss.detach().cpu()),
                **loss_bd.scalars(),
            }
...
def train_group_classifier(
    ...
    eval_registry = eval_registry or registry
    return train_classifier(
        model,
        registry.factory(group_name),
        config,
        objective=objective,
        eval_batch_fn=eval_registry.factory(group_name),
    )
